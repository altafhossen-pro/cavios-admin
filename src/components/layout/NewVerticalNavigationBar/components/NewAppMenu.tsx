...
const NewMenuItemWithChildren = ({ item, className, linkClassName, subMenuClassName, activeMenuItems, toggleMenu }: SubMenus) => {
              ...
              return (
                <Fragment key={child.key + idx}>
                  {child.children ? (
                    ...
                    <NewMenuItem item={child} className="sub-nav-item" linkClassName={clsx('sub-nav-link', getActiveClass(child))} />
                  )}
                </Fragment>
              )
            })}
...
const NewMenuItem = ({ item, className, linkClassName }: SubMenus) => {
  return (
    <li className={className}>
      <NewMenuItemLink item={item} className={linkClassName} />
    </li>
  )
}

const NewMenuItemLink = ({ item, className }: SubMenus) => {
  return (
...
type NewAppMenuProps = {
  menuItems: Array<MenuItemType>
}

const NewAppMenu = ({ menuItems }: NewAppMenuProps) => {
            ...
            {item.isTitle ? (
              ...
              <>
                {item.children ? (
                  ...
                  <NewMenuItem item={item} linkClassName={clsx('nav-link', getActiveClass(item))} className="nav-item" />
                )}
              </>
            )}
          </Fragment>
