...
import { getStaticPageById, updateStaticPage, UpdateStaticPageRequest, checkSlugAvailability } from '@/features/admin/api/staticPageApi'
import TinyMCEEditor from '@/components/wrappers/TinyMCEEditor'

const EditStaticPagePage = () => {
  ...
  }, [pageId, navigate, showNotification])
  ...
  useEffect(() => {
    ...
    slugCheckTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        const response = await checkSlugAvailability(formData.slug || '', pageId)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const err = error as { message?: string }
      showNotification({
        message: err.message || 'Failed to update static page',
        variant: 'danger',
      })
    } finally {
      setLoading(false)
  ...
  if (fetching) {
    return (
      <>
        <PageMetaData title="Edit Static Page" />
        <PageBreadcrumb title="Edit Static Page" subName="Ecommerce" />
        <Card>
          <CardBody>
...
export default EditStaticPagePage
